...
class Code{
    ...
    private $code = null;
    private $nums = 0;
    public function __construct($nums=5){
        $this->nums = $nums;
    ...
    //imgcode的function
    function imgcode($width,$high, $nums=4) {
        ...
        for ($i = 0; $i < $this->nums; $i++) {
            $strpos = rand(1, 6);
            imagestring($image, 5, $strx, $strpos, substr($this->code, $i, 1), $black);
            $strx += rand(10, 30);
        }
